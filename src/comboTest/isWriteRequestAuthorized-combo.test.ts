import { WriteRequestAuthorizedRequest } from 'fhir-works-on-aws-interface';
...
describe('isWriteRequestAuthorized-combo', () => {
    ...
    test.each(testCases)('CASE: %s', async (testCaseString, testCase) => {
        let testResult: any;
        try {
            testResult = await authZHandler.isWriteRequestAuthorized(<WriteRequestAuthorizedRequest>testCase.request);
